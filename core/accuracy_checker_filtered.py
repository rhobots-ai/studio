#!/usr/bin/env python3
"""
Accuracy Checker for Invoice Prediction Model (Filtered Version)
===============================================================

This version excludes empty predictions from accuracy calculations,
providing more realistic accuracy metrics for actual prediction attempts.
"""
...
class FilteredAccuracyChecker:
    """Accuracy checker that excludes empty predictions from calculations."""
    ...
    def process_records(self, max_records: Optional[int] = None) -> None:
        """Process records and calculate accuracy metrics, excluding empty predictions."""
        print("Processing records (excluding empty predictions)...")
        ...
        for i, record in enumerate(self.data[:records_to_process]):
            ...
            try:
                ...
                if not predict_text:
                    self.results['empty_predictions_excluded'] += 1
                    continue
                
                # Count this as a record with prediction
                self.results['records_with_predictions'] += 1
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("=" * 70)
        report.append("FILTERED INVOICE PREDICTION ACCURACY REPORT")
        report.append("(Excluding Empty Predictions)")
        report.append("=" * 70)
        
        # Overall Statistics
        total = self.results['total_records']
        with_predictions = self.results['records_with_predictions']
        excluded = self.results['empty_predictions_excluded']
        
        report.append(f"\nOVERALL STATISTICS:")
        report.append(f"Total Records: {total:,}")
        report.append(f"Records with Predictions: {with_predictions:,} ({with_predictions/total*100:.1f}%)")
        report.append(f"Empty Predictions Excluded: {excluded:,} ({excluded/total*100:.1f}%)")
        report.append(f"JSON Parsing Success: {self.results['json_parsing_success']:,} ({self.results['json_parsing_success']/with_predictions*100:.1f}% of attempts)")
        report.append(f"Perfect Record Matches: {self.results['exact_matches']:,} ({self.results['exact_matches']/with_predictions*100:.1f}% of attempts)")
        
        # Field-level Accuracies
        report.append(f"\nFIELD-LEVEL ACCURACIES (Based on Non-Empty Predictions Only):")
        report.append("-" * 60)
        ...
        report.append("=" * 50)
        report.append(f"By excluding {excluded:,} empty predictions:")
        report.append(f"- Accuracy calculations are now based on {with_predictions:,} actual attempts")
        report.append(f"- This provides more realistic performance metrics")
        report.append(f"- Focus is on model performance when it actually tries to predict")
    ...
    def save_detailed_results(self, output_file: str = "filtered_accuracy_results.json") -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Check accuracy of invoice predictions (excluding empty predictions)")
    ...
    parser.add_argument("--output", "-o", default="filtered_accuracy_results.json",
    ...
    checker = FilteredAccuracyChecker(args.file)
